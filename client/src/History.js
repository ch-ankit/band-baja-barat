...
function History({ userId }) {
  ...
  let orders = [];
  useEffect(() => {
    const historyUrl = admin
      ? "http://localhost:9000/giftstore/orders?userName=admin"
  ...
  if (history !== undefined && history.length !== 0) {
    history.forEach((element) => {
      const index = Object.keys(data).findIndex(
        (items) => data[items].modelNo === element.giftId
      );
      index !== -1 &&
        orders.push({
          ...element,
  ...
  return (
    <div className="historz">
      <img
        className="history__ad"
        src="https://laz-img-cdn.alicdn.com/images/ims-web/TB1os6RgSslXu8jSZFuXXXg7FXa.jpg_1200x1200Q100.jpg"
        alt="history__ad"
      />
      {history !== undefined && history.length === 0 ? (
        <div>
          ...
          <p>
            You have no items in your history. To add one go and purchase an
            item
          </p>
        </div>
      ) : (
